...
"""Build `results/thesis/final_summary/` — every number, table and figure, in one pass.
    ...
    python deployment/code/scripts/build_final_summary.py --no-client-eval     # skip per-hospital scoring
...
def discover(only: str | None = None) -> list[Run]:
    ...
    for experiment, d in alvos:
        ...
        if run.predictions is None or run.predictions.empty:
            run.detail = ("no predictions_test.csv — run deployment/code/scripts/collect_results.py "
...
def comparisons(table: pd.DataFrame) -> dict[str, pd.DataFrame]:
    ...
    # FedAvg against FedProx is a PAIRED comparison. Each FedProx run shares its partition with exactly
...
def write_markdown(path: Path, runs: list[Run], table: pd.DataFrame,
    ...
    md = [
        ...
        "`deployment/code/scripts/build_final_summary.py`.",
        "",
...
Status: {n_done} of {n_total} experiments present.{missing_note}
...
| Column | What it is |
|---|---|
| `macro_auc` | one-vs-rest ROC AUC, averaged over the {n_classes} classes |
...
| `macro_precision`, `macro_recall`, `macro_f1` | unweighted mean over the {n_classes} classes |
...
| `best_round` | federated runs, the round maximising `{key_metric}` across clients |
...
def write_readme(path: Path, runs: list[Run], cohort_sites: pd.DataFrame) -> None:
    """Write final_summary/README.md. `cohort_sites` is unused, kept for the caller."""
    done = [r for r in runs if r.ok]
    missing = [r.id for r in runs if not r.ok]
    rows = "\n".join(
        f"| `{r.id}` | {r.algorithm} | {r.experiment.n_clients} | "
        f"{r.experiment.split_label} |"
        for r in runs)
...
def main() -> None:
    ...
    if args.require_complete and n_done < len(runs):
        raise SystemExit(
            ...
            "results.\n  Run them, then deployment/code/scripts/collect_results.py, then this again.")
