...
"""Divide the training patients between hospitals. One physical folder per site.
    ...
    python deployment/code/scripts/partition_data.py                     # all four partitions
    python deployment/code/scripts/partition_data.py --only 4_clients_skewed
...
the previous run of these experiments detected no effect from quantity skew.
...
strongest heterogeneity available. It needs a pooled source dataset.
"""
...
def build_partition(name: str, partition, rows: pd.DataFrame, patients: pd.DataFrame,
    ...
    # client uses them only when class_weight_scope == "global".
