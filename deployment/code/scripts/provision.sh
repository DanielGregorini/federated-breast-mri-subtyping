...
#
# WHAT IT WRITES
#     deployment/workspace/breast_fl_project/prod_NN/
...
#     nvflare provision -p deployment/project.yml -w deployment/workspace
#
...
CODE_ROOT="$(dirname "$HERE")"                  # deployment/code, holds config/
DEPLOYMENT_DIR="$(dirname "$CODE_ROOT")"        # deployment/
WORKSPACE_DIR="$DEPLOYMENT_DIR/workspace"
PROJECT_YML="$DEPLOYMENT_DIR/project.yml"
...
echo
echo "next: start the server and the hospitals from their startup kits"
