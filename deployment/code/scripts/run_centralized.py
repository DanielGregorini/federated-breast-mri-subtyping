...
"""Test 01 — the centralised baseline. Not an NVFLARE job.
    ...
    python deployment/code/scripts/run_centralized.py
    python deployment/code/scripts/run_centralized.py --seed 1 --epochs 30
...
same number of times on both sides. Without that, the comparison would be reading a difference
...
# the configuration this shares with the federated clients is one level up
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Every line also lands in deployment/logs/test01/train.log, timestamped. The
