...
`deployment/logs/<test>/admin.log`, timestamped. That file is the admin side of the
record the dissertation reconstructs a run from — the server and each hospital write
their own files beside it, written by each participant's startup kit.

The federation must already be running. See deployment/README.md. This script
...
class _Tee:
    ...
    `deployment/logs/<test>/admin.log` as well, with a UTC timestamp, so the admin
...
def preflight(experiment) -> None:
    ...
    if missing:
        raise SystemExit(
            ...
            f"data: {', '.join(missing)}\n"
            "  run: python deployment/code/scripts/partition_data.py")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if experiment.kind == "centralized":
        raise SystemExit(
            ...
            "  run: python deployment/code/scripts/run_centralized.py")
    ...
    print("next: python deployment/code/scripts/collect_results.py")
