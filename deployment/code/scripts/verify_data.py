...
def verify_global() -> set:
    ...
    if test is None:
        check(False, "data/global/test.csv exists",
              "run deployment/code/scripts/prepare_data.py first")
        return set()
...
def verify_partition(name: str, test_pids: set) -> None:
    ...
    if not part_dir.is_dir():
        check(False, f"{name} exists", "run deployment/code/scripts/partition_data.py")
        return
...
def verify_imports() -> None:
    """The shared trainer must not import nvflare.

    That is the invariant the layout rests on: the centralised baseline and every
    federated client run the same code, so neither tree may reach for the runtime
    only one of them has.
    """
    print("\nshared code purity")
    trees = [EX.REPO_ROOT / "src", PROJECT_ROOT / "common"]
    for tree in trees:
        rel = tree.relative_to(EX.REPO_ROOT)
        if not tree.is_dir():
            check(False, f"{rel}/ exists to be checked")
            continue
        offenders = []
        files = sorted(tree.rglob("*.py"))
        for path in files:
            for i, line in enumerate(path.read_text().splitlines(), 1):
                if line.strip().startswith(("import nvflare", "from nvflare")):
                    offenders.append(f"{path.relative_to(EX.REPO_ROOT)}:{i}")
        check(not offenders,
              f"no file in {rel}/ imports nvflare ({len(files)} files)",
              "; ".join(offenders))
