...
"""Everything that must be true before a single epoch is trained.
    ...
    python deployment/code/scripts/verify_production.py --strict     # warnings become failures
...
def verify_structure() -> None:
    ...
    for sub in ("code", "config", "jobs", "datasets", "logs",
                "workspace", "data"):
...
def verify_provisioning() -> None:
    ...
    check(ws.parent.parent == EX.WORKSPACE_DIR,
          "workspace sits under deployment/workspace/",
...
def verify_ports() -> None:
    ...
    for label, port in (("fed_learn", FED.FED_LEARN_PORT), ("admin", FED.ADMIN_PORT)):
        ...
        check(not busy, f"{label} port {port} is free",
              ...
              "a participant is still running, or it is an old federation",
              warn_only=True)
...
def verify_experiments_and_jobs() -> None:
    ...
    # them cannot be compared against its counterpart there.
    #
    ...
    # The centralised baseline has no job folder. It is not an NVFLARE job.
    for e in [x for x in EX.EXPERIMENTS if x.kind == "federated"]:
        d = EX.JOBS_DIR / e.name
        check((d / "meta.json").is_file(), f"{e.id}: meta.json")
        for cfg in ("config_fed_server.json", "config_fed_client.json"):
            check((d / "app" / "config" / cfg).is_file(), f"{e.id}: {cfg}")
        # The job has to carry the client, or the hospital has nothing to run.
        check((d / "app" / "custom" / "federation" / "client.py").is_file(),
              f"{e.id}: app/custom/federation/client.py")
        # An absolute path in a job config is a path on THIS machine. The job is
        # submitted by name from the admin console and has to mean the same thing
        # wherever it lands.
        cfg_text = "".join(
            f.read_text() for f in (d / "app" / "config").glob("*.json"))
        check(str(EX.REPO_ROOT) not in cfg_text,
              f"{e.id}: job config holds no absolute path")
    ...
    for pname, partition in EX.PARTITIONS.items():
        ...
        check(got == sorted(CORE_ALGOS),
              ...
              "a configuration missing FedAvg or FedProx has nothing to compare against")

    # The centralised baseline must be the SAME configuration as the federated
    # clients, or the measured gap is partly a hyperparameter difference. They share one
    ...
    check(r.returncode == 0, "every job matches config/experiments.py",
          "run: python deployment/code/scripts/generate_jobs.py   (a job was edited by hand)")
...
def verify_splits() -> None:
    ...
    for name, partition in EX.PARTITIONS.items():
        ...
        # are not two splits of one dataset, and the comparison would be between two datasets.
...
def verify_outputs() -> None:
    ...
    check(EX.LOGS_DIR.is_dir(), "deployment/logs/ exists")
    check(EX.FIGURES_DIR.is_dir(), "results/federated/distributions/ exists")
    ...
    check(len(figures) == len(EX.EXPERIMENTS),
          ...
          "run: python deployment/code/scripts/build_distribution_report.py")
    for e in EX.EXPERIMENTS:
    ...
    check((EX.DATASETS_DIR / "all_distributions.csv").is_file(),
          "deployment/datasets/all_distributions.csv")
    check((EX.DATASETS_DIR / "all_distributions.json").is_file(),
          "deployment/datasets/all_distributions.json")
