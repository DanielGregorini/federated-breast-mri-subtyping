...
10 vs 12 and 11 vs 13 are the MATCHED PAIRS. All four hold 642,
...
WHICH COMPARISON EACH TEST BELONGS TO
-------------------------------------
    federated against centralised     Test 01 against Tests 02-09
    cohort heterogeneity              Test 10 against 12, Test 11 against 13
                                      (Tests 06 against 08 is quantity skew only)
    FedAvg against FedProx            every even/odd pair. The one that matters is
                                      10 against 11, where FedProx has real
                                      heterogeneity to correct
"""
...
# genuinely non-IID. Quantity skew across
...
# --------------------------------------------------------------------------- #
# `deployment/` is the deployment and output layer: the provisioning file, the PKI
# workspace, the thirteen jobs, and everything a run emits. It holds no second
# definition of any hyperparameter. The jobs under `deployment/jobs/` are GENERATED
# from this file by `deployment/code/scripts/generate_jobs.py`.
#
...
# looked wrong. So configuration lives here, once.
...
# client. That is what makes the comparison fair: if the federated runs used
...
class TrainingConfig:
    ...
    compared to is the single easiest way to make the comparison meaningless, so they share
...
PARTITIONS: dict[str, Partition] = {
    ...
    # ----------------------------------------------------------------------- #
    # THE HETEROGENEITY PAIR — one cohort per site, and its size-matched control #
    # ----------------------------------------------------------------------- #
    ...
    # Kairouz et al., and it is why quantity skew alone measures so little.
    #
...
EXPERIMENTS: list[Experiment] = [
    ...
    #
    # test10-test13 below are the heterogeneity experiments that DID complete.

    # ----------------------------------------------------------------------- #
    # THE HETEROGENEITY PAIR                                                   #
    # ----------------------------------------------------------------------- #
    ...
    # heterogeneity test14 introduces, so this is where FedProx has its strongest
