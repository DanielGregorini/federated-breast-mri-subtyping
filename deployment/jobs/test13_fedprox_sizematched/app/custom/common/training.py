...
other exists, which is what lets the same code serve both, and what makes the comparison a
...
def train_one_epoch(model, loader, criterion, optimizer, scaler, device, *,
    ...
    curves FedAvg and FedProx are compared on, and would make the number drift as `mu` changes rather
...
def lr_for_round(base_lr: float, current_round: int, num_rounds: int,
    ...
    rate every time. Either turns the comparison into one of learning-rate schedules.
