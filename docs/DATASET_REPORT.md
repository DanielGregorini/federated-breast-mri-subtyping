...
figure is regenerated by `src/scripts/build_dataset_report_figures.py` from the same
...
1. Fridman, N., Solway, B., Fridman, T. et al. **BreastDCEDL: A standardised deep
...
`src/scripts/build_dataset_report_figures.py` resolves both locations so the figures can
